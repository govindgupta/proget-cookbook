...
property :proget_resource_name, String, name_property: true
...
action :install do
  ...
  package 'ProGet' do # ~FC009
    ...
    options args.join(' ')
    notifies :run, "ruby_block[#{new_resource.proget_resource_name} install failure]", :immediately
    notifies :run, "ruby_block[wait for #{new_resource.proget_resource_name}]", :immediately
  end if args

  ruby_block "#{new_resource.proget_resource_name} install failure" do
    action :nothing
    block { raise 'Installation of Proget failed' }
    not_if { current_version == new_resource.version }
  end

  ruby_block "wait for #{new_resource.proget_resource_name}" do
    action :nothing
    block do
      require 'net/http'
      ...
      until test_server.call
        Chef::Log.info "Waiting for #{new_resource.proget_resource_name} to be up"
        sleep 2
      end
    end
  end
